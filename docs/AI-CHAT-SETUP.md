...
- OpenRouter: default model `openrouter/free`. This router selects an available free model, so a new key can work without credits. OpenRouter model IDs use the `provider/model` format; you can replace this with a paid model after adding credits.
